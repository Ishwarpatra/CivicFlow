...
## Neon Free PostgreSQL preparation — 23 August 2026

The repository now contains a staged PostgreSQL data layer that activates only when `POSTGRES_DATABASE_URL` (or a PostgreSQL-valued `DATABASE_URL`) is configured. The existing deployed service remains on its SQLite fallback because no Neon secret has been supplied and no cutover has been approved. The staged path runs PostgreSQL schema migrations for account records, route progress, saved briefings, guide quotas, election data, and a PostgreSQL-backed Express session store; the legacy local SQLite behavior remains the test and no-secret fallback.

The local quality gates passed with **10 test files and 63 tests**, `npm run build`, `npm run lint`, and `git diff --check`. Two `pg-mem` regressions exercise the PostgreSQL schema, ownership uniqueness, and conflict-safe election seeding without contacting a public database. A guarded import command is also available for a protected SQLite snapshot; it refuses a non-empty user-owned target and intentionally does not migrate Express sessions. This is preparation evidence only, not a live Neon connection or production data-import validation.
